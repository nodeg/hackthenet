...
switch($action) 
{
  ...
  */
  ...
  $usr['points_pc']=number_format(floor($usr['points']/$pccnt), 0, ',', '.');
  ...
  if ($c!==false) 
  {
    ...
    echo '<p>Dein Cluster besitzt <strong>'.$c['points'].' Punkte</strong> ( durschnittlich <strong>'.$usr['points_pc'].' pro</strong> PC ). Damit ist dein Cluster auf dem <strong>'.$c['rank'].'. Platz</strong> in der Gesamtwertung.</p>
  ...
  case 'gewinnspiel':  // ------------------------- Gewinnspiel für HackTheNet ------------------------
  // Header
  createlayout_top('HackTheNet - - Gewinnspiel');
  include('gewinnspiel.php');
  //--- Ausgabe des Footers
  createlayout_bottom();
	break;
...
if($found==true) {
  ...
  while($x=mysql_fetch_assoc($sql))
  {
    ...
    if($ext) 
    {
      ...
      {
	  	if($usr['bigacc']=='yes') {
        $mmstat='<br /><a href="game.php?m=item&amp;item=mm&amp;sid='.$sid.'&amp;xpc='.$x['id'].'"><span style="color:red">MoneyMarket-Update verf&uuml;gbar!</span></a>';
		}
		else {
      ...
      elseif($avail) {
	  	  	if($usr['bigacc']=='yes') {
	  		$attack='<a href="battle.php?m=opc&amp;sid='.$sid.'&amp;xpc='.$x['id'].'"><span style="color:green">m&ouml;glich</span></a>';
			}
			else {
  ...
  #setuserval('pcs', $list);
  #echo $list;
  $durch_point = floor($tpoint / $number );
  $durch_credit = number_format(floor($tcreds / $number ), 0, ',', '.');
...
<tr>
<td>Durschnitt</td>
<td class="points">'.$durch_point.' Punkte</td>
<td class="credits">'.$durch_credit.' Credits</td>
</tr>
</table>
...
$notiz_save = mysql_query("UPDATE users SET notiz=\"$notiz\" WHERE id=\"$usrid\"");
...
for($i=0;$i<count($lines);$i++) {
  if(trim($lines[$i])!='') {
    ...
    switch(strtoupper($lines[$i][0])) {
      ...
      case 'NOTUSER': $notuser[$nuv]=$lines[$i][1]; $nuv++; break;
      case 'NOTCLUSTER': $notcluster[$ncv]=$lines[$i][1]; $ncv++; break;

      case 'USER': $suser[$suv]=$lines[$i][1]; $suv++; break;
      case 'CLUSTER': $scluster[$scv]=$lines[$i][1]; $scv++; break;

      case 'POINTS':
        switch(strtoupper(trim($lines[$i][1])))
        {
        case '>=': $gg=1; $points=trim($lines[$i][2]); break;
        case '<=': $kg=1; $points=trim($lines[$i][2]); break;
        case '>': $groesser=1; $points=trim($lines[$i][2]); break;
        case '<': $kleiner=1; $points=trim($lines[$i][2]); break;
        case '=': $gleich=1; $points=trim($lines[$i][2]); break;
        default:
...
foreach($notuser as $i => $j) {
if ($j == "0") {
if ($i=="1") { $searchvars="owner!=\"0\" "; }
else { $searchvars.="AND owner_!=\"0\" "; }
}
else {
if ($i=="1") { $searchvars="owner_name!=\"$j\" "; }
else { $searchvars.="AND owner_name!=\"$j\" "; }
}
}
foreach($notcluster as $i => $j) {
if ($j == "0") {
if ($notuser[1]=="") { $searchvars="owner_cluster!=\"0\" "; }
else { $searchvars.="AND owner_cluster!=\"0\" "; }
}
else {
if ($notuser[1]=="") { $searchvars="owner_cluster_code!=\"$j\" "; }
else { $searchvars.="AND owner_cluster_code!=\"$j\" "; }
}
}
foreach($suser as $i => $j) {
if ($suser[$i] == "0") {
if ($i=="1" && $notcluster[1]!="" || $notuser[1]!="") { $searchvars.="AND owner=\"0\" "; }
if ($i=="1" && $notcluster[1]=="" && $notuser[1]=="") { $searchvars="owner=\"0\" "; }
if ($i!="1") { $searchvars.="OR owner_name=\"0\" "; }
}
else {
if ($i=="1" && $notcluster[1]!="" || $notuser[1]!="") { $searchvars.="AND owner_name=\"$j\" "; }
if ($i=="1" && $notcluster[1]=="" && $notuser[1]=="") { $searchvars="owner_name=\"$j\" "; }
if ($i!="1") { $searchvars.="OR owner_name=\"$suser[$i]\" "; }
}
}
foreach($scluster as $i => $j) {
if ($$j == "0") {
if ($i=="1" && $suser[1]!="" || $notcluster[1]!="" || $notuser[1]!="") { $searchvars.="AND owner_cluster=\"0\" "; }
if ($i=="1" && $suser[1]=="" && $notcluster[1]=="" && $notuser[1]=="") { $searchvars="owner_cluster=\"0\" "; }
if ($i!="1") { $searchvars.="OR owner_cluster_code=\"0\" "; }
}
else {
if ($i=="1" && $suser[1]!="" || $notcluster[1]!="" || $notuser[1]!="") { $searchvars.="AND owner_cluster_code=\"$j\" "; }
if ($i=="1" && $suser[1]=="" && $notcluster[1]=="" && $notuser[1]=="") { $searchvars="owner_cluster_code=\"$j\" "; }
if ($i!="1") { $searchvars.="OR owner_cluster_code=\"$scluster[$i]\" "; }
}
}
if ($gg=="1" || $kg=="1" || $groesser=="1" || $kleiner=="1") {
if (($scluster[1]!="" || $suser[1]!="" || $notcluster[1]!="" || $notuser[1]!="") && $gg=="1") { $searchvars.="AND points>=\"$points\" "; }
if (($scluster[1]=="" && $suser[1]=="" && $notcluster[1]=="" && $notuser[1]=="") && $gg=="1") { $searchvars="points>=\"$points\" "; }
if (($scluster[1]!="" || $suser[1]!="" || $notcluster[1]!="" || $notuser[1]!="") && $kg=="1") { $searchvars.="AND points<=\"$points\" "; }
if (($scluster[1]=="" && $suser[1]=="" && $notcluster[1]=="" && $notuser[1]=="") && $kg=="1") { $searchvars="points<=\"$points\" "; }
if (($scluster[1]!="" || $suser[1]!="" || $notcluster[1]!="" || $notuser[1]!="") && $kleiner=="1") { $searchvars.="AND points<\"'.$points\" "; }
if (($scluster[1]=="" && $suser[1]=="" && $notcluster[1]=="" && $notuser[1]=="") && $kleiner=="1") { $searchvars="points<\"$points\" "; }
if (($scluster[1]!="" || $suser[1]!="" || $notcluster[1]!="" || $notuser[1]!="") && $groesser=="1") { $searchvars.="AND points>\"$points\" "; }
if (($scluster[1]=="" && $suser[1]=="" && $notcluster[1]=="" && $notuser[1]=="") && $groesser=="1") { $searchvars="points>\"$points\" "; }
if (($scluster[1]!="" || $suser[1]!="" || $notcluster[1]!="" || $notuser[1]!="") && $gleich=="1") { $searchvars.="AND points=\"$points\" "; }
if (($scluster[1]=="" && $suser[1]=="" && $notcluster[1]=="" && $notuser[1]=="") && $gleich=="1") { $searchvars="points=\"$points\" "; }
}  

/* echo $searchvars; */
if ($searchvars == "") { echo "<div class=\"important\"><h3>Suche</h3><p>Keine Suchkriterie angegeben!</p></div>"; exit; }
$searchquery = mysql_query("select * from pcs WHERE ".$searchvars." ORDER by points DESC");
$search_found_pcs = mysql_num_rows($searchquery);
echo "<div class=\"important\"><h3>Suche</h3><p>".$search_found_pcs." PCs gefunden.</p></div>";
...
echo "</tr>";
while ($sqrow = mysql_fetch_array($searchquery)) {
$search_owner = $sqrow['owner'];
$search_owner_name = $sqrow['owner_name'];
$search_owner_cluster = $sqrow['owner_cluster'];
$search_owner_cluster_code = $sqrow['owner_cluster_code'];
$search_pc_name = $sqrow['name'];
$search_pc_points = $sqrow['points'];
$search_pc_ip = $sqrow['ip'];
if ($no_ranking_users!=$search_owner && $no_ranking_clusters!=$search_cluster) {
if ($search_owner == "0") { $search_if_nick = "NoName"; }
else { $search_if_nick = '<a href="user.php?a=info&amp;user='.$search_owner.'&amp;sid='.$sid.'">'.$search_owner_name.'</a>'; }
if ($search_cluster == "0") { $search_if_cluster = ""; }
else { $search_if_cluster = '<a href="cluster.php?a=info&amp;sid='.$sid.'&amp;cluster='.$search_owner_cluster.' ">'.$search_owner_cluster_code.'</a>'; }
echo "<tr>";
echo "<td class=\"name\">$search_if_nick</td><td class=\"cluster\">$search_if_cluster</td><td class=\"ip\">10.47.$search_pc_ip</td><td class=\"pcname\">$search_pc_name</td><td class=\"pcpoints\">$search_pc_points</td>";
echo "</tr>";
}
}
