...
case 'clustermsg': //------------------------- Clustermessage -------------------------------
if ($_POST['new'] == "1") {
	$rec=trim('Dein Cluster');
	...
	if(strlen($text)>5120) {
    	...
    	if($text=='') {
      		...
      		if($usr[lastmail]+15 <= $ts || $localhost || $usrid==1) {
				...
				$sql = mysql_query('SELECT `id` FROM `users` WHERE `cluster` ='.$clusterid.'');
            	while($row = mysql_fetch_array($sql)) {
            		db_query('UPDATE users SET liu=\''.mysql_escape_string($ts).'\' WHERE id=\''.mysql_escape_string($row['id']).'\'');
            		db_query('INSERT INTO mails VALUES (\'\', \''.mysql_escape_string($row['id']).'\', \'Dein Cluster\', \'Dein Cluster\', \''.mysql_escape_string($ts).'\', \''.mysql_escape_string($subject).'\', \''.mysql_escape_string($text).'\', \'in\', \'no\')');
            	...
            	$ok.='Die Nachricht wurde erfolgreich an deinen Cluser verschickt!'."\n";
	...
	if($e==false) {
		createlayout_top('HackTheNet - Messages');
  		...
  		<div class=ok>
  		<h3>Erfolgreich</h3>
  		<p>Deine Clustermitteilung wurde Erfolgreich abgeschickt!<br>
  		Bitte achte darauf nichts unwichtiges ans Cluster zu schreiben..</p>
  		</div>';
		createlayout_bottom();
	} 
	else {
  		...
  		createlayout_top('HackTheNet - Messages');
  		echo '<div class="content" id="messages">'.LF.'<h2>Cluster</h2>'.LF.$err."\n";
  		echo '<div id="messages-compose">
			  <h3>Mail verfassen</h3>
			  <form action="cluster.php?a=clustermsg&amp;sid='.$sid.'" method="post">
		...
		<th>Betreff:</th>
		<td><input name="subject" type="text" value="'.$_REQUEST['subject'].'" /></td>
		</tr>
		<tr id="messages-compose-text">
		<th>Text:</th>
		<td><textarea name="text" rows="5" cols="50" value="'.$_REQUEST['text'].'></textarea></td>
		</tr>
...
if($usr['clusterstat'] == CS_ADMIN || $usr['clusterstat'] == CS_COADMIN)
{
  ...
  <tr>
<th>Angriff auf<br /> Vertragspartner: </th>
  <td>
  <br>';
  if ( $cluster['battle'] == "3" ) {
  echo '<input type="radio" name="battle" value="3" checked="checked"/> Angriffe immer Zulassen
  <br>';
  }
  else {
  echo '<input type="radio" name="battle" value="3"/> Angriffe immer Zulassen
  <br>';
  }
  if ( $cluster['battle'] == "2" ) {
  echo '<input type="radio" name="battle" value="2" checked="checked"/> Bei Vertragsbruch nur Warnen
  <br>';
  }
  else {
  echo '<input type="radio" name="battle" value="2"/> Bei Vertragsbruch nur Warnen
  <br>';
  }
  if ( $cluster['battle'] == "1" ) {
  echo '<input type="radio" name="battle" value="1" checked="checked"/> Vertragsbruch Verhindern </td>';  
  }
  else {
  echo '<input type="radio" name="battle" value="1"/> Vertragsbruch Verhindern </td>';  
  }
echo '</tr>
  <tr>
  <td colspan="2"><input type="submit" value="Speichern" /></td>
...
if($_POST['delete']=='yes')
{
  ...
  if($usr['clusterstat'] != CS_ADMIN)
  {
    simple_message('Nur Clusteradmins können Cluster löschen!');
    exit;
  }
...
else
{
  ...
  if($e==true) {
	...
	$cluster['battle']=safeentities($battle);
    savemycluster();
...
| <a href="secret.php?sid='.$sid.'&page=delcluster&id='.$c.'">Cluster löschen!!</a>';
}
