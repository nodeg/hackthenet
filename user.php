...
switch($action) 
{
  ...
  <a href="abook.php?mode=admin&amp;sid='.$sid.'">Adressbuch verwalten</a>
    ...
    $url = 'http://htn.fun-synchro.de/usrimg.php/'.$server.'-'.$usrid.'.png';
  ...
  if($usr['bigacc']=='yes') 
  {
    ...
    <td>';
	if ( $usr['dropmenu'] == "1" ) {
  	echo '<input type="radio" name="dropmenu" value="1" checked="checked"/> Aktivieren
  	<br>';
  	}
  	else {
  	echo '<input type="radio" name="dropmenu" value="1"/> Aktivieren
  	<br>';
  	}
  	if ( $usr['dropmenu'] == "2" ) {
 	echo '<input type="radio" name="dropmenu" value="2" checked="checked"/> Deaktivieren
  	<br>';
  	}
  	else {
  	echo '<input type="radio" name="dropmenu" value="2"/> Deaktivieren
  	<br>';
  	}
    echo '</td>
    </tr>';
  ...
  {
    echo '<tr id="settings-settings-usrimg">
    <th>PC Dropdownmenü:</th>
    <td>Die Funktion steht nur Extendedaccounts zur Verfügung</td>
    </tr>';
  }
  ...
  if($_POST['delete_account'] == 'yes') 
  {
    ...
    if(!@mail($usr['email'],'HackTheNet-Account löschen?',$body,'From: robot@htn.fun-synchro.de')) 
    {
      echo nl2br($body);
    }
