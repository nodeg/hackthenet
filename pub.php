...
switch($action) 
{
  ...
  Wenn du dieses Feld leer lässt, wird dir ein Passwort vom System generiert.</td>
  </tr>';
  ######## by BODY-SNATCHER
  ######## Dropdownliste entfernt
  #echo '<tr><th>Land für deinen PC:</th><td><select name="country">
  #';
  ...
  if(check_email($email)===true) 
  {
    ...
    if($usr!==false) 
    {
      if($email==strtolower($usr['email']))
      {
        ...
        if(@mail($email,'Zugangsdaten für HackTheNet',LF.'http://www.htn-game.tk/'.LF.LF.'Server: Server '.$server.LF.'Benutzername: '.$usr['name'].LF.'Passwort: '.$pwd.LF,'From: HackTheNet <robot@htn.fun-synchro.de>')) 
        {
