...
return window.confirm(\'Mail wirklich einem Administrator zeigen? Wer eine Falschmeldung schickt wird verwarnt.\');
}
';
...
function newmailcount($save=true) {
  ...
  $anz+=mysql_num_rows($result);
  if($save) db_query('UPDATE users SET newmail=\''.$anz.'\' WHERE id=\''.$usrid.'\';');
  return $anz;
}
...
$usr_cache;
$maillist_mcnt;
function maillist($boxid) { // ------------------------- MAIL LIST -----------------------------
  global $STYLESHEET, $REMOTE_FILES_DIR, $DATADIR, $usrid,$sid,$full;
  global $usr_cache,$maillist_mcnt;
  $output='';
  $mcnt=0;

  $result=db_query('SELECT * FROM mails WHERE user=\''.mysql_escape_string($usrid).'\' AND box=\''.mysql_escape_string($boxid).'\' ORDER BY time DESC');
  $anz=mysql_num_rows($result);
...
function newmailform($recip='',$subject='',$text='',$xnl2br=true) { // -------- NEW MAIL FORM -----------
...
if($usr['bigacc']=='yes') $bigacc='<tr id="messages-compose-address-book">'.LF.'<td colspan="2"><input type="button" value="erweitert..." onclick="show_maile(\'user\');"/><input type="button" value="Adressbuch..." onclick="show_abook(\'user\');"/></td>'.LF.'</tr>'."\n";
echo '<div id="messages-compose">
<h3>Mail verfassen</h3>';
//if ($usr['bigacc']!='yes' ) echo "<p><span style='color:red;'>Du besitzt keinen extendend Account, darum kannst du das Adressbuch und die erweiterten Funktionen nicht benutzen</span></p>";
echo '<form action="mail.php?a=sendmail&amp;sid='.$sid.'" method="post" name="newmail">
<table>
...
while($data=mysql_fetch_assoc($result)):
  ...
  if($data[xread]=='yes') { $new=''; $ch=''; } else { $new=' class="not-read"'; $ch='checked '; }
...
$arcstate=(int)@mysql_num_rows(db_query('SELECT mail FROM mails WHERE user='.mysql_escape_string($usrid).' AND box=\'arc\';')).'/'.getmaxmails('arc');
...
<p><a href="mail.php?a=ignorelist&amp;sid='.$sid.'">Ignore List</a> | <a href="user.php?a=bbcode&amp;sid='.$sid.'">BB Code</a></p>
</div>
'.$notif.$full;
if ($inbox!='') {
...
header('Location: mail.php?m='.$_REQUEST['redir'].'&sid='.$sid.'&ok='.urlencode('Keine Mails ausgew&auml;hlt.'));
}
break;

case 'archive': //------------------------- ARCHIVE -------------------------------
$cnt=@mysql_num_rows(db_query('SELECT * FROM mails WHERE user='.mysql_escape_string($usrid).' AND box=\'arc\''));
if($cnt<getmaxmails('arc')) {
  $box=$_REQUEST[type];
  $s=getcheckedmail_str($box);
  if (!$s) { $error='Es wurden keine Mails ausgewählt'; }
  else {
...
$tousers=explode(',', $rec);
$admins=explode(',', $no_ranking_users);

if (count($tousers)==1) { $recip=getuser($rec,'name'); $rec=$recip[name]; }
else { $recip=true; }

if($recip!=false) {
  if (mysql_num_rows(mysql_query('SELECT * FROM `msg_ignore` WHERE user="'.$recip['id'].'" && user2="'.$usrid.'"')) > 0 && !is_noranKINGuser($usrid)) { 
        $e=true; $error.='Der User hat dich auf der Ignoreliste.'; 
  } else { 
     if (strlen($text)>5120) {
        $e=true;
        $error.='Die Nachricht darf maximal 5120 Zeichen haben.';
     } else {
     if (count($tousers)>32 && in_array($usrid, $admins)=="") {
        $e=true;
        $error.='Die Nachricht darf maximal an 32 Leute geschickt werden.';
     } else {
        if($text=='') {
           $e=true;
        ...
        db_query('INSERT INTO mails VALUES (\'0\', \''.mysql_escape_string($recip['id']).'\', \''.mysql_escape_string($usrid).'\', \''.mysql_escape_string($usr['name']).'\', \''.mysql_escape_string($ts).'\', \''.mysql_escape_string($subject).'\', \''.mysql_escape_string($text).'\', \'in\', \'no\')');
        db_query('INSERT INTO mails VALUES (\'0\', \''.mysql_escape_string($usrid).'\', \''.mysql_escape_string($recip['id']).'\', \''.mysql_escape_string($recip['name']).'\', \''.mysql_escape_string($ts).'\', \''.mysql_escape_string($subject).'\', \''.mysql_escape_string($text).'\', \'out\', \'yes\')');

        $r=db_query('SELECT mail FROM mails WHERE user='.mysql_escape_string($usrid).' AND box=\'out\' ORDER BY time ASC');
        ...
        db_query('UPDATE users SET lastmail=\''.time().'\' WHERE id=\''.mysql_escape_string($usrid).'\'');
        ...
        $text=mail_format_text($text);
        if(strlen($subject)>255) $subject=substr($subject,0,255);
        for ($x=0; $x<count($tousers); $x++) {
        $recip=getuser($tousers[$x],'name');
        $r=db_query('SELECT mail FROM mails WHERE user='.mysql_escape_string($recip['id']).' AND box=\'in\'');
        $cnt=mysql_num_rows($r);
        if($cnt<getmaxmailsforuser('in',$recip['bigacc'])) {
        ...
        $ok.='Die Nachricht wurde erfolgreich an '.$tousers[$x].' verschickt!'."\n";
        db_query('UPDATE users SET lastmail=\''.time().'\' WHERE id=\''.mysql_escape_string($usrid).'\'');
        } else {
         $e=true; $error.='Das Postfach von '.$tousers[$x].' ist leider voll. Du kannst ihm/ihr keine Nachricht schicken.<br />'.$tousers[$x].' hat f&uuml;r diesen Fall aber folgende Nachricht hinterlassen:<br />'.$recip['inbox_full'];
        }
        } 
        }
  } else {
...
$result=db_query('SELECT * FROM mails WHERE USER=\''.mysql_escape_string($usrid).'\' AND mail LIKE \''.mysql_escape_string($msg).'\' LIMIT 1;');
...
if($data['box']=='in')
  $links.=' | <a href="mail.php?a=markunread&amp;c'.$msg.'=1&amp;sid='.$sid.'&amp;redir='.$redir.'&amp;type=in">Als neu markieren</a><br><br>';
  $links.='<br><a href="mail.php?a=warn&amp;msg='.$msg.'&amp;sid='.$sid.'&amp;" onclick="return confirm_abort();">Message einem Game-Administrator zeigen</a> | <a href="mail.php?a=ignore&amp;user='.$sender['id'].'&amp;sid='.$sid.'&amp;">Absender ignorieren</a>';
...
case 'warn': //------------------------- WARN -------------------------------
if ($no_ranking_users!="") {
	$admins=explode(",", $no_ranking_users); $admin='';
	for ($i=0; $i<count($admins); $i++) { 
		if (is_numeric($admins[$i])) { $admin=$admins[$i]; $i=count($admins); }
	}
	if ($admin=="") { simple_message('Es wurde kein Administrator gefunden.'); exit; }
	$msg=$_REQUEST['msg'];
	$data=mysql_fetch_assoc(db_query('SELECT * FROM mails WHERE mail LIKE \''.mysql_escape_string($msg).'\';'));
	$ts=time();
	$text='Geschrieben von '.$data['user2_name'].':<br /><br />';
	$text.=$data['text'];
	db_query('INSERT INTO mails VALUES (\'0\', \''.mysql_escape_string($admin).'\', \''.mysql_escape_string($data['user']).'\', \''.mysql_escape_string($usr['name']).'\', \''.mysql_escape_string($ts).'\', \'Gemeldete MSG: '.$data['subject'].'\', \''.mysql_escape_string($text).'\', \'in\', \'no\')');
	$newmail=mysql_fetch_assoc(mysql_query('SELECT newmail FROM users WHERE id='.mysql_escape_string($admin).''));
	$newmails=$newmail['newmail'];
	$newmails++;
	db_query('UPDATE users SET newmail="'.$newmails.'" WHERE id='.mysql_escape_string($admin).'');
	for ($i=0; $i<count($admins); $i++) { 
		if (is_numeric($admins[$i]) && $admin!=$admins[$i]) { AddSysMsg($admins[$i], 'Der Administrator Account hat eine neue Nachricht.'); }
	}
	createlayout_top('HackTheNet - Messages');
	echo '<div class="important"><h3>Mail</h3><p>Mail wurde erfolgreich dem Administrator geschickt.</p></div>';
	echo '</div>'."\n";
	createlayout_bottom();
} else { simple_message('Es wurde kein Administrator gefunden.'); }
break;

case 'ignore': //------------------------- IGNORE -------------------------------
$ignore=$_REQUEST['user'];
if (mysql_num_rows(mysql_query('SELECT * FROM msg_ignore WHERE user="'.mysql_escape_string($usrid).'" && user2="'.mysql_escape_string($ignore).'"')) == 0) {
   db_query("INSERT INTO `msg_ignore` ( `id` , `user` , `user2` ) VALUES ('', '".mysql_escape_string($usrid)."', '".mysql_escape_string($ignore)."')");
   createlayout_top('HackTheNet - Messages');
   $absender=getuser($ignore);
   echo '<div class="important"><h3>Mail</h3><p>Absender ('.$absender['name'].') wurde auf die Ignoreliste gesetzt.</p></div>';
   echo '</div>'."\n";
   createlayout_bottom();
} else {
   createlayout_top('HackTheNet - Messages');
   echo '<div class="important"><h3>Mail</h3><p>Du kannst einen User nicht mehrmals auf die Ignoreliste setzen.</p></div>';
   echo '</div>'."\n";
   createlayout_bottom();
}
break;
...
$ignorelist=mysql_query('SELECT * FROM msg_ignore WHERE user="'.mysql_escape_string($usrid).'"');
...
while ($ignore=mysql_fetch_array($ignorelist)) {
$ignoreuser=getuser($ignore['user2']);
echo '<tr><td><a href="user.php?a=info&amp;user='.$ignore['user2'].'&amp;sid='.$sid.'&amp;">'.$ignoreuser['name'].'</a></td><td><a href="mail.php?a=ignoredel&amp;user='.$ignore['user2'].'&amp;sid='.$sid.'&amp;">Löschen?</a></td></tr>';
}
echo '</table></div>'."\n";
createlayout_bottom();
} else { simple_message('Es steht keiner auf deiner Ignore Liste'); }
break;

case 'ignoredel': //------------------------- IGNORE DELETE -------------------------------
$ignore=$_REQUEST[user];
if (mysql_num_rows(mysql_query('SELECT * FROM msg_ignore WHERE user="'.mysql_escape_string($usrid).'" && user2="'.mysql_escape_string($ignore).'"')) > 0) {
   db_query('DELETE FROM msg_ignore WHERE user="'.mysql_escape_string($usrid).'" && user2="'.mysql_escape_string($ignore).'"');
   createlayout_top('HackTheNet - Messages');
   $absender=getuser($ignore);
   echo '<div class="important"><h3>Mail</h3><p>User ('.$absender['name'].') wurde von der Ignoreliste gelöscht.</p></div>';
   echo '</div>'."\n";
   createlayout_bottom();
} else {
   createlayout_top('HackTheNet - Messages');
   echo '<div class="important"><h3>Mail</h3><p>Du hast diesen User nicht auf der Ignoreliste.</p></div>';
   echo '</div>'."\n";
   createlayout_bottom();
}
break;
...
function transmit2_mail_list($box) {
  ...
  $result=db_query('SELECT * FROM mails WHERE user=\''.mysql_escape_string($usrid).'\' AND box=\''.mysql_escape_string($box).'\' ORDER BY time DESC');
  $anz=mysql_num_rows($result);
  while($data=mysql_fetch_assoc($result)):
      $mcnt++;
      $sender=GetUser($data['user2']);
...
if(file_exists($fn)==false) { simple_message('FEHLER!'); exit; }
...
while($data=mysql_fetch_assoc($r)):
  ...
  $fn=$ofn;  $x=0;
  while(isset($list[$fn])) { $x++; $fn=$ofn.'_'.$x; }
...
function getcheckedmail_str($boxid,$prefix='c') {
  ...
  settype($a,'array');
  $result=db_query('SELECT mail FROM mails WHERE user=\''.mysql_escape_string($usrid).'\' AND box=\''.mysql_escape_string($boxid).'\'');
