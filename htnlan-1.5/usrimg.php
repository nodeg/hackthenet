...
if(file_exists($imgfile)!=true) {
  ...
  imagestringup($hImg, 5, 1, $h-6, 'HTN.LAN', $darkred);
  imagestring($hImg, 3, 30, 2, $usr['name'], $black);
  imagestringup($hImg, 1, 15, $h-10, 'by Schnitzel', $black);
  $currenty=24;
  if(stristr($option,'cluster')!=false) { imagestring($hImg, 2, 30, $currenty, $c['code'], $black); $currenty+=12; }
  if(stristr($option,'points')!=false) { imagestring($hImg, 2, 30, $currenty, $usr['points'].' Punkte', $black); $currenty+=12; }
  if(stristr($option,'ranking')!=false) { imagestring($hImg, 2, 30, $currenty, 'Platz '.$usr['rank'], $black); $currenty+=12; }
