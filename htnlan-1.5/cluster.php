...
function savemycluster() { # Eigenen Cluster speichern
...
$reqs=@mysql_num_rows(db_query('SELECT user FROM cl_reqs WHERE cluster='.mysql_escape_string($clusterid).' AND dealed=\'no\''));
$funcs='';
...
$members=mysql_num_rows(db_query('SELECT id FROM users WHERE cluster=\''.mysql_escape_string($clusterid).'\''));
...
$cboard_result=db_query('SELECT cluster FROM cboards WHERE time>'.$usr['lca'].' && cluster='.$cluster['id']);
$cboard_numrows=mysql_num_rows($cboard_result);

echo '<div id="cluster-overview">
<h3>'.$cluster['name'].'</h3>
<table width="90%">
'.$img.'<tr id="cluster-overview-board1">
<td colspan="2"><a href="cboard.php?page=board&amp;sid='.$sid.'">Zum Cluster-Board '.($cboard_numrows>0 ? '(<i>'.$cboard_numrows.' neue Nachricht'.($cboard_numrows>1 ? 'en' : '').'</i>)' : '').'</a></td>
</tr>
<tr>
<th>Name:</th>
<td>'.$cluster['name'].'</td>
...
if ($cluster['ircnetwork']!="") {
echo '<tr>
<th>IRC Network:</th>
<td>'.$cluster['ircnetwork'].'</td>
</tr>';
}
if ($cluster['ircchannel']!="") {
echo '<tr>
<th>IRC Channel:</th>
<td>'.$cluster['ircchannel'].'</td>
</tr>';
}
if ($cluster['externb']!="nein") {
echo '<tr>
<th>Externes Forum:</th>
<td>'.$cluster['boardurl'].'</td>
</tr>';
}
echo '<tr>
...
$conventlist = conventlist($cluster['id']);
echo '<div id="cluster-notice-create">
<h3>Aktuelle Verträge</h3>'.($conventlist=="" ? '<p>Keine Verträge vorhanden.</p>' : $conventlist).'</div>';
...
$sql='FROM cl_pacts WHERE cluster='.mysql_escape_string($clusterid).' AND partner='.mysql_escape_string($c[1]).' AND convent='.mysql_escape_string($c[0]).' LIMIT 1';
...
if(@mysql_num_rows($r)==1) {
  ...
  db_query('UPDATE clusters SET events=\''.mysql_escape_string($cluster['events']).'\' WHERE id='.mysql_escape_string($clusterid));
}
...
$r=db_query('SELECT cl_pacts.convent,clusters.code,clusters.id,cl_pacts.partner FROM (cl_pacts RIGHT JOIN clusters ON cl_pacts.partner=clusters.id) WHERE cl_pacts.cluster='.mysql_escape_string($clusterid).' ORDER BY clusters.code ASC;');
...
$r=db_query('SELECT cl_pacts.convent,clusters.code,clusters.id FROM (cl_pacts RIGHT JOIN clusters ON cl_pacts.cluster=clusters.id) WHERE cl_pacts.partner='.mysql_escape_string($clusterid).' ORDER BY clusters.code ASC;');
if(mysql_num_rows($r) > 0) {
echo '<div id="cluster-convents">
...
if($dat==false) {
  ...
  $result = db_query('SELECT cluster FROM cl_pacts WHERE cluster="'.mysql_escape_string($clusterid).'" && convent="'.mysql_escape_string($type).'" && partner="'.$dat['id'].'" LIMIT 1;');
  if (mysql_num_rows($result)==1) { $error='Es kann nicht zwei mal der gleiche Vertrag eingetragen werden!'; }
  elseif (mysql_num_rows($result)>1) { db_query('DELETE FROM cl_pacts WHERE cluster="'.mysql_escape_string($clusterid).'" && convent="'.mysql_escape_string($type).'" && partner="'.$dat['id'].'"'); }
  else {
     $type=(int)$_POST['type'];
     ...
     db_query('UPDATE clusters SET events=\''.mysql_escape_string($dat['events']).'\' WHERE id='.mysql_escape_string($dat['id']));
     db_query('INSERT INTO cl_pacts VALUES ('.mysql_escape_string($clusterid).', '.mysql_escape_string($type).', '.mysql_escape_string($dat['id']).');');
...
simple_message("<p><b>Du kannst den Betrag noch nicht verändern!. Erst wieder um am ". date("d.m.Y",$new_round_time + NO_TRANSFER)." um ". date("H:i",$new_round_time + NO_TRANSFER)."</b></p>");
exit;
}
if($usr['clusterstat']==CS_ADMIN || $usr['clusterstat']==CS_COADMIN) {
$tax=$_REQUEST['tax'];
if(is_long((int)$tax)) {
  $cluster['events']=nicetime4().' [usr='.$usrid.']'.$usr['name'].'[/usr] setzt Mitgliedsbeitrag auf '.$tax.' Credits pro Tag'.LF.$cluster['events'];
  db_query('UPDATE clusters SET events=\''.mysql_escape_string($cluster['events']).'\',tax='.mysql_escape_string($tax).' WHERE id='.mysql_escape_string($clusterid));
...
<tr>
<th>Betrag:</th>
<td><input type="text" name="credits" value="0" /> Credits</td>
</tr>
<tr>
...
$r=db_query('SELECT id,name,cm FROM users WHERE cluster=\''.mysql_escape_string($clusterid).'\' ORDER BY name ASC');
...
$r=db_query('SELECT * FROM users WHERE cluster=\''.mysql_escape_string($clusterid).'\' ORDER BY name ASC');

while($udat=mysql_fetch_assoc($r)):
  $uix=$udat['id'];
  $cstat=cscodetostring($udat['clusterstat']);
  ...
  echo '</select></td>'.LF.'<td><input type="text" maxlenght="50" name="title'.$uix.'" value="'.($udat['clustertitle']!="" && $udat['clustertitle']!=$cstat ? $udat['clustertitle'] : $cstat).'"></td>'.LF.'<td>'.nicetime3($udat['login_time']).'</td>'.LF.'<td><input type="checkbox" value="yes" name="kick'.$uix.'" /></td></tr>';
endwhile;
...
$r=db_query('SELECT id,name,clusterstat FROM users WHERE cluster=\''.mysql_escape_string($clusterid).'\' ORDER BY name ASC');
...
while($udat=mysql_fetch_assoc($r)):
  ...
  if ($_POST['kick'.$uix]=='yes') { # User aus dem Cluster schmei&szlig;en?
    ...
    $title=$_REQUEST['title'.$uix];
    if($udat['clusterstat']!=$stat || $udat['clustertitle']!=$title) {
      db_query('UPDATE users SET clusterstat=\''.mysql_escape_string($stat).'\', clustertitle=\''.mysql_escape_string($title).'\' WHERE id='.mysql_escape_string($uix));
      $cluster['events']=nicetime4().' [usr='.$udat['id'].']'.$udat['name'].'[/usr] erh&auml;lt durch [usr='.$usrid.']'.$usr['name'].'[/usr] den Status '.cscodetostring($stat).''.($title!="" ? ' ('.mysql_escape_string($title).')' : '').'.'.LF.$cluster['events'];
    }
  }
endwhile;
...
<tr>
<th>IRC-Netzwerk:</th>
<td><input type="text" name="ircnetwork" maxlength="30" value="'.$cluster['ircnetwork'].'" /></td>
</tr>
<tr>
<th>IRC-Channel:</th>
<td><input type="text" name="ircchannel" maxlength="30" value="'.$cluster['ircchannel'].'" /></td>
</tr>
<tr>
<th>Externes Board:</th>';
if ($cluster['externb']=="ja") { $m=' checked="checked"'; }
if ($cluster['externb']=="nein") { $w=' checked="checked"'; }
echo '<td><input type="radio" name="externb" value="ja" id="ja"'.$m.' />Ja <input type="radio" name="externb" value="nein" id="nein"'.$w.' />Nein </td>
</tr>
<tr>
<th>Boardurl:</th>
<td><input type="text" name="boardurl" value="'.($cluster['boardurl']=='' ? 'http://' : $cluster['boardurl']).'" /></td>
</tr>
<tr>
<th>Beschreibung:<br />BB-Code: Eingeschaltet. <a href="game.php?m=kb&amp;sid='.$sid.'#messages-compose" target="_blank">BB-Codes</a></th>
...
Ordner 3:<br />
<input type="text" name="box2" value="'.$cluster['box3'].'" maxlength="30" /><br />
Ordner 4 (admin only):<br />
<input type="text" name="box3" value="'.$cluster['box4'].'" maxlength="30" /></td>
</tr>
<tr>
<th>Logo-Datei:</th>
...
if($_POST['delete']=='yes') {
  ...
  db_query('INSERT INTO logs SET type=\'delcluster\', usr_id=\''.mysql_escape_string($usrid).'\', payload=\''.mysql_escape_string($usr['name']).' deletes '.mysql_escape_string($cluster['code']).'\';');
}
break;
...
if ($_POST['externb']=="ja" && $_POST['boardurl']=="") { $e=true; $msg.='Wenn du ein externes Board verwendest muss auch eine URL angegeben sein.<br />'; }
if(eregi('http://.*',$_POST['boardurl'])==false) { $_POST['boardurl']=''; }
...
$cluster['box4']=safeentities($_POST['box3']);
$cluster['name']=$name;
$cluster['code']=$code;
...
$cluster['boardurl']=safeentities($_POST['boardurl']);
$cluster['ircnetwork']=safeentities($_POST['ircnetwork']);
$cluster['ircchannel']=safeentities($_POST['ircchannel']);
$cluster['externb']=safeentities($_POST['externb']);
savemycluster();
...
db_query('INSERT INTO rank_clusters VALUES(0,'.mysql_escape_string($id).',1,'.mysql_escape_string($usr['points']).','.mysql_escape_string($usr['points']).','.mysql_escape_string($pcs).','.mysql_escape_string($pcs).',0)');
...
$r=db_query('SELECT * FROM cl_reqs WHERE cluster='.mysql_escape_string($x['id']).' AND dealed=\'yes\' AND user='.mysql_escape_string($usrid));
if(@mysql_num_rows($r)<1) { simple_message('Der Antrag ist abgelaufen!'); exit; }
db_query('DELETE FROM cl_reqs WHERE cluster='.mysql_escape_string($x['id']).' AND dealed=\'yes\' AND user='.mysql_escape_string($usrid));
...
if($oldcluster!==false) {
  ...
  db_query('UPDATE clusters SET events=\''.mysql_escape_string($oldcluster['events']).'\' WHERE id='.mysql_escape_string($oldcluster['id']).';');
}
...
$r=db_query('SELECT id FROM users WHERE cluster='.mysql_escape_string($clusterid).' AND clusterstat='.(CS_ADMIN).';');
...
while($member=mysql_fetch_assoc($r)):
  if($member!==false) {
    ...
    if(file_exists('data/login/'.$member['sid'].'.txt')==true) $online='<span style="color:green;">Online</span>';
      else $online='<span style="color:red;">Offline</span>';
    $members.='<tr>'.LF.'<td><a href="user.php?page=info&amp;user='.$member['id'].'&amp;sid='.$sid.'">'.$member['name'].'</a></td>'.LF.'<td>'.cscodetostring($member['clusterstat']).''.($member['clustertitle']!="" && $member['clustertitle']!=cscodetostring($member['clusterstat']) ? ' ('.stripslashes($member['clustertitle']).')' : '').'</td>'.LF.'<td>'.$member['points'].'</td>'.LF.'<td>'.$online.'</td>'.LF.'<td>'.$lli.'</td>'.LF.'</tr>'."\n";
  }
  $lli='';
endwhile;
...
$ts=time()-24*60*60;
$r=db_query('SELECT * FROM attacks WHERE '.($dir=='in'?'to_cluster':'from_cluster').'='.mysql_escape_string($clusterid).' AND time>='.mysql_escape_string($ts).' ORDER BY time DESC;');
...
echo '<p><a name="attackfrom"><strong>Angriffe <em>durch</em> Mitglieder des Clusters</strong></a></p>'."\n";
echo '<p>jump to: <a href="#attackto"><i>Angriff auf Mitglieder des Clusters</i></a></p>'."\n";
battle_table('out');
echo '<br /><p><a name="attackto"><strong>Angriffe <em>auf</em> Mitglieder des Clusters</strong></a></p>'."\n";
echo '<p>jump to: <a href="#attackfrom"><i>Angriff von Mitgliedern des Clusters</i></a></p>'."\n";
battle_table('in');
...
$credits=str_replace('all', ($cluster['money']-1), $credits);
...
if($dat[0]=='user') {
  ...
  $recip_usr=getUser($recip['owner']);
  $cluster['events']=nicetime4().' [usr='.$usrid.']'.$usr['name'].'[/usr] hat '.$dat[2].' Credits an [usr='.$recip_usr['id'].']'.$recip_usr['name'].'[/usr] überwiesen.'.LF.$cluster['events'];
  ...
  $c=getcluster($dat[1]);
  $c['money']+=$dat[3];
  $cluster['events']=nicetime4().' [usr='.$usrid.']'.$usr['name'].'[/usr] überweist '.$dat[3].' Credits an den Cluster [cluster='.$c['id'].']'.$c['code'].'[/cluster]'.LF.$cluster['events'];
  $c[events]=nicetime4().' Der Cluster [cluster='.$clusterid.']'.$cluster['code'].'[/cluster] überweist dem Cluster '.$dat[3].' Credits.'.LF.$c['events'];
...
db_query('INSERT INTO transfers VALUES(\''.mysql_escape_string($clusterid).'\', \'cluster\', \'0\', \''.mysql_escape_string($dat[1]).'\', \''.mysql_escape_string($dat[0]).'\', \''.mysql_escape_string($recip['owner']).'\', \''.mysql_escape_string($dat[3]).'\', \''.time().'\');');
...
db_query('INSERT INTO cl_reqs VALUES(\''.mysql_escape_string($usrid).'\', \''.mysql_escape_string($c['id']).'\', \''.nl2br(safeentities($_POST['comment'])).'\', \'no\');');
...
if($acstr!='') {
  ...
  db_query('UPDATE cl_reqs SET dealed=\'yes\' WHERE ('.$acstr.') AND cluster='.mysql_escape_string($clusterid));
}
...
db_query('UPDATE clusters SET notice=\''.mysql_escape_string($n).'\' WHERE id='.mysql_escape_string($clusterid).';');
...
function conventlist($cid) {
	...
	$r=mysql_query('
	SELECT
	cl_pacts.convent,clusters.code,clusters.id,cl_pacts.partner,cl_pacts.cluster
	FROM (cl_pacts RIGHT JOIN clusters ON cl_pacts.partner=clusters.id) 
	WHERE 
	cl_pacts.cluster='.mysql_escape_string($cid).' 
	...
	SELECT
	cl_pacts.convent,clusters.code,clusters.id,cl_pacts.partner,cl_pacts.cluster
	FROM (cl_pacts RIGHT JOIN clusters ON cl_pacts.cluster=clusters.id) 
	WHERE 
	cl_pacts.partner='.mysql_escape_string($cid).' 
	ORDER BY clusters.code ASC;
	');
	...
	if (mysql_num_rows($r)>0 && mysql_num_rows($r2)>0) {
		$s='<table>'.LF.'<tr>'.LF.'<th>Cluster</th>'.LF.'<th>Vertrag</th>'.LF.'</tr>'."\n";
                $pcluster=array();
		while($pact=mysql_fetch_assoc($r)) {
			$pact['convent']=cvcodetostring($pact['convent']);
			$pcluster[]=$pact;
		}
		$ppartner=array();
		while ($pact2=mysql_fetch_assoc($r2)) {
			$pact2['convent']=cvcodetostring($pact2['convent']);
			$ppartner[]=$pact2;
		}
                $convents=array();
		for($i=0; $i<count($pcluster); $i++) {
			for ($x=0; $x<count($ppartner); $x++) {
				if (($ppartner[$x]['partner'] == $pcluster[$i]['cluster']) && ($ppartner['cluster'] == $pcluster['partner']) && ($ppartner[$x]['convent'] == $pcluster[$i]['convent']) && (!array_search($pcluster[$i]['cluster'].$pcluster[$i]['partner'].$pcluster[$i]['convent'],$convents)) && (!array_search($pcluster[$i]['partner'].$pcluster[$i]['cluster'].$pcluster[$i]['convent'],$convents))) {
                                        $convents[]=$pcluster[$i]['cluster'].$pcluster[$i]['partner'].$pcluster[$i]['convent'];
					$s.='<tr>'.LF.'<td><a href="cluster.php?page=info&amp;sid='.$sid.'&amp;cluster='.$pcluster[$i]['id'].'">'.$pcluster[$i]['code'].'</a></td>'.LF.'<td>'.$pcluster[$i]['convent'].'</td>'.LF.'</tr>'."\n";
				}
			}
		}
		$s.='</table>';
...
function conventlist2($cid) { // ----------- CONVENTLIST -----------
global $STYLESHEET, $REMOTE_FILES_DIR, $DATADIR, $clusterid,$cluster,$sid;
