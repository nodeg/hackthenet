...
if($usr['da_avail']!='yes') { simple_message('Noch nicht verf&uuml;gbar!'); exit; }
...
<td class="participants">';
                ...
                $i=0;
                echo '<a id="+" onClick="javascript:ToggleSubmenu('.$item['id'].'); return false;" href="#"><img id="Menu_'.$item['id'].'" src="static/plus.jpg" border="0">&nbsp;&nbsp;show/hide pcs</a><div id="Sub_'.$item['id'].'" style="display:none;">';
...
if (is_norankinguser($target['owner'])) { $e.='Du kannst den Administrator des Spiels nicht angreifen.'; }
if (!is_pc_attackable($target, $owner) && $target['owner']!=0) { $e.='Gegen diesen schwachen PC kann kein DA ausgeführt werden!!'; }
...
echo '<div class="ok"><h3>Aktion ausgef&uuml;hrt</h3>'.LF.'<p>Die Distributed Attack wurde erstellt.</p>'.LF.'</div><br />
  <p><a href="distrattack.php?page=list&amp;sid='.$sid.'">Weiter</a></p>';
list($target,$credits,$item)=explode('|',file_get($fn));
@unlink($fn);
db_query('INSERT INTO distr_attacks VALUES(\'0\', \''.mysql_escape_string($clusterid).'\', \''.mysql_escape_string($pcid).'\', \''.mysql_escape_string($usrid).'\', \''.mysql_escape_string($target).'\', \''.mysql_escape_string($item).'\');');
$id=mysql_insert_id();
db_query('INSERT INTO da_participants SET relative='.mysql_escape_string($id).', pc='.mysql_escape_string($pc['id']).', ip=\''.mysql_escape_string($pc['ip']).'\', owner='.mysql_escape_string($usrid).', owner_name=\''.mysql_escape_string($usr['name']).'\';');
#echo mysql_error();
$pc['credits']-=$credits;
...
if(isset($a['id'])==true && $a['cluster']==$clusterid) {
  ...
  while($x=mysql_fetch_assoc($sql)):
    if(isavailh('da',$x)!=true) continue;
      $result=db_query('SELECT relative FROM da_participants WHERE relative=\''.mysql_escape_string($a['id']).'\' AND pc=\''.mysql_escape_string($x['id']).'\';');
...
if(isset($a['id'])==true && $a['cluster']==$clusterid) {
  ...
  while($xpc=mysql_fetch_assoc($sql)):
        ...
        db_query('INSERT INTO da_participants SET relative=\''.mysql_escape_string($a['id']).'\', pc=\''.mysql_escape_string($xpc['id']).'\', ip=\''.mysql_escape_string($xpc['ip']).'\', owner=\''.mysql_escape_string($usrid).'\', owner_name=\''.mysql_escape_string($usr['name']).'\';');
  endwhile;
...
if(isset($a['id'])==true && $a['cluster']==$clusterid) {
  ...
  if($a['initiator_usr']!=$usrid && $usr['clusterstat']!=CS_ADMIN) { exit; }
  ...
  if($total >= 1) {
	...
	if ($owner)
	{
		if(is_pc_attackable($remote,$owner)==false)
		{
			echo '<div class="error"><h3>N00b-Schutz</h3><p>Gegen diesen schwachen PC kann kein DA ausgeführt werden!!</p></div>';
		  	createlayout_bottom();
		  	exit; 
		}
...
############
      ...
      if($a[item]=='cpu') {
        ...
        if($lv>0) {
          $lv=$cpu_levels[$lv];
        } elseif($remote[cpu]==$cpu_levels[0]) { $lv=$cpu_levels[0]; $nx=true; } else $lv=$cpu_levels[0];
      } else {
        ...
        } elseif($remote[$a[item]]==1) { $lv=1; $nx=true; } else $lv=1;
      }
...
############
    ...
    if($dif<10 || $remote[$a['item']]==$lv || ($a[item]=='cpu' && $remote[$a['item']]==array_search($lv,$cpu_levels)) ) {
      ...
      addsysmsg($owner['id'],$total.' Mitglieder des Clusters [cluster='.$clusterid.']'.$cluster['code'].'[/cluster] haben eine Distributed Attack gegen dich ausgef&uuml;hrt!<br />Sie konnte aber abgewehrt werden: Auf deinem PC 10.47.'.$remote['ip'].' ('.$remote['name'].') gab es keinen Schaden!');
    } else {
      ...
      addsysmsg($remote['owner'],$total.' Mitglieder des Clusters [cluster='.$clusterid.']'.$cluster['code'].'[/cluster] haben eine Distributed Attack gegen dich ausgef&uuml;hrt!<br />Sie war erfolgreich: Auf deinem PC 10.47.'.$remote['ip'].' ('.$remote['name'].') wurde '.idtoname($a['item']).' auf '.$lv.' zerst&ouml;rt!');
    }
    ...
    while($b=mysql_fetch_assoc($result)) {
      $uix=$b['owner'];
      if($sended[$uix]!=true) {
        $dbresult=db_query('SELECT danotice FROM users WHERE id='.mysql_escape_string($uix).' LIMIT 1;');
	if (mysql_result($dbresult,0,'danotice') == 'yes') {
        	addsysmsg($uix,$msg);
        	$sended[$uix]=true;
	}
      }
