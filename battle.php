...
<script language="JavaScript" type="text/javascript">
      function fill(s) {
        document.frm.code.value+=\'TARGET 10.47.\'+s;
      }
      function addcode(s) {
        document.frm.code.value+=s+\'\n\';
      }
</script>';
...
if($usr['bigacc']=='yes') {
	if(isavailh('scan',$pc)==true) { 
  		$weapons.='<tr class="greytr2"><td nowrap="nowrap">REMOTE SCAN</td><td><a onclick="javascript:addcode(\'REMOTE SCAN \');"><tt>REMOTE SCAN</tt></a></td><td>Spioniert fremde Rechner aus</td><td>Waffe</td></tr>';
  	}
  	if(isavailh('trojan',$pc)==true) {
    	$tEx='<table style="font-size:8pt;" class="nomargin">';
    	if(tisavail('defacement')==true) {
			$tEx.='<tr><td nowrap="nowrap"><a onclick="javascript:addcode(\'>TROJAN DEFACEMENT \');"><tt>TROJAN DEFACEMENT</tt></a></td><td>Du kannst die Beschreibung des gegnerischen Users &auml;ndern</td></tr>';
		}
    	if(tisavail('transfer')==true) {
			$tEx.='<tr><td nowrap="nowrap"><a onclick="javascript:addcode(\'>TROJAN TRANSFER \');"><tt>TROJAN TRANSFER</tt></a></td><td>Klaut Geld</td></tr>';
		}
    	if(tisavail('deactivate')==true) {
			$tEx.='
			<tr><td nowrap="nowrap"><a onclick="javascript:addcode(\'>TROJAN DEACTIVATE FIREWALL \');"><tt>TROJAN DEACTIVATE FIREWALL</tt></a></td><td>Deaktiviert die Firewall</td></tr>
        	<tr><td nowrap="nowrap"><a onclick="javascript:addcode(\'>TROJAN DEACTIVATE AV \');"><tt>TROJAN DEACTIVATE AV</tt></a></td><td>Deaktiviert das Antivirus-Program</td></tr>
        	<tr><td nowrap="nowrap"><a onclick="javascript:addcode(\'>TROJAN DEACTIVATE FIREWALL \');"><tt>TROJAN DEACTIVATE IDS</tt></a></td><td>Deaktiviert das IDS</td></tr>';
		}
   		$tEx.='</table>';
  	...
  	if(isavailh('smash',$pc)==true) { 
  		$weapons.='
		<tr class="greytr2">
			<td nowrap="nowrap">REMOTE SMASH [ITEM]</td>
			<td nowrap>
			<a onclick="javascript:addcode(\'>REMOTE SMASH CPU \');"><tt>REMOTE SMASH CPU</tt></a><br /> oder
    		<a onclick="javascript:addcode(\'>REMOTE SMASH FIREWALL \');"><tt>REMOTE SMASH FIREWALL</tt></a><br /> oder
    		<a onclick="javascript:addcode(\'>REMOTE SMASH SDK \');"><tt>REMOTE SMASH SDK</tt></a>
    		</td>
			<td>Zerst&ouml;rt Prozessor (CPU), Firewall (FIREWALL) oder SDK (SDK) von fremden Rechnern</td>
			<td>Waffe</td>
		</tr>';
	}
  	if(isavailh('block',$pc)==true) { 
  		$weapons.='
		<tr class="greytr2">
			<td nowrap="nowrap">REMOTE BLOCK</td>
			<td><a onclick="javascript:addcode(\'>REMOTE BLOCK \');"><tt>REMOTE BLOCK</tt></a></td>
			<td>Blockiert Computer f&uuml;r dessen Besitzer</td><td>Waffe</td>
		</tr>'; }

  	if(isavailh('rh',$pc)==true) { 
		$weapons.='
		<tr class="greytr2">
			<td nowrap="nowrap">REMOTE HIJACK</td>
			<td><a onclick="javascript:addcode(\'>REMOTE HIJACK \');"><tt>REMOTE HIJACK</tt></a></td>
			<td>Versucht, den Computer des Gegners zu &uuml;bernehmen</td><td>Waffe</td>
		</tr>'; }
}
else {
...
$kill_friends=mysql_fetch_assoc(mysql_query("SELECT * FROM clusters WHERE id='".$usr[cluster]."'"));

if ( $kill_friends['battle'] == "1" ) {
	if (@mysql_num_rows($showvertraege)>0) { $e.='Du kannst keine Vertragspartner angreifen!'; }
	if ($usr[cluster] == $target_owner[cluster]) { $e.='Du kannst keine Clusterkollegen angreifen!'; }
}
...
if($hijack) {
 ...
 $userpcs = explode(",", $usr['pcs']);

if (count($userpcs) >= 10) {

 $usrname = $usr['name'];
 $abfragex=@mysql_num_rows(db_query('SELECT * FROM pcs WHERE owner_name = "'.$usrname.'" AND points = "1024"'));
 if ($abfragex < 5) {
         $e.='Du musst mindestens 5 PCs voll ausgebauen, damit du weiter PCs hijacken kannst!';
     }
  }
if (count($userpcs) >= 20) {
 ...
 $abfragex=@mysql_num_rows(db_query('SELECT * FROM pcs WHERE owner_name = "'.$usrname.'" AND points = "1024"'));
 if ($abfragex < 10) {
         $e.='Du musst mindestens 10 PCs voll ausgebauen, damit du weiter PCs hijacken kannst!';
     }
  }
if (count($userpcs) >= 50) {
 ...
 $abfragex=@mysql_num_rows(db_query('SELECT * FROM pcs WHERE owner_name = "'.$usrname.'" AND points = "1024"'));
 if ($abfragex < 25) {
         $e.='Du musst mindestens 25 PCs voll ausgebauen, damit du weiter PCs hijacken kannst!';
     }
  }
if (count($userpcs) >= 100) {

 $usrname = $usr['name'];
 $abfragex=@mysql_num_rows(db_query('SELECT * FROM pcs WHERE owner_name = "'.$usrname.'" AND points = "1024"'));
 if ($abfragex < 50) {
         $e.='Du musst mindestens 50 PCs voll ausgebauen, damit du weiter PCs hijacken kannst!';
     }
  }
if (count($userpcs) >= 500) {

 $usrname = $usr['name'];
 $abfragex=@mysql_num_rows(db_query('SELECT * FROM pcs WHERE owner_name = "'.$usrname.'" AND points = "1024"'));
 if ($abfragex < 250) {
         $e.='Du musst mindestens 250 PCs voll ausgebauen, damit du weiter PCs hijacken kannst!';
     }
  }
}
...
if(($e=='' && $scan==0) & ($owner['points']<MIN_ATTACK_POINTS && $owner['login_time']+MIN_INACTIVE_TIME>time())) $e.='Du kannst keinen User mit weniger als '.$MIN_ATTACK_POINTS.' Punkten angreifen, der sich in den letzten 3 Tagen eingeloggt hat!<br />';

if($e=='')
{
if ( $kill_friends['battle'] == "2")  {
 	if (@mysql_num_rows($showvertraege)>0) { 
	$text='<h3>Angriff best&auml;tigen <font color="red">!!! Achtung angriff gegen einen Vertragspartner !!!</font></h3><table>
	<tr><th>Angriffsdetails</th></tr>
	<tr><td>
    <ul><li><b>Der Angriff richtet sich gegen 10.47.'.$target['ip'].' ('.$target['name'].'). Dieser Rechner geh&ouml;rt ';
	}
 	if ($usr['cluster'] == $target_owner['cluster']) { 
	$text='<h3>Angriff best&auml;tigen <font color="red">!!! Achtung angriff gegen einen Clustermitglied !!!</font></h3><table>
	<tr><th>Angriffsdetails</th></tr>
	<tr><td>
	<ul><li><b>Der Angriff richtet sich gegen 10.47.'.$target['ip'].' ('.$target['name'].'). Dieser Rechner geh&ouml;rt ';
	}
...
if ( $kill_friends['battle'] == "2")  {
 	if (@mysql_num_rows($showvertraege)>0) { 
	$text.='<input type="submit" value="Trotzdem angreifen!" /></th></tr>';
	}
 	if ($usr['cluster'] == $target_owner['cluster']) { 
	$text.='<input type="submit" value="Trotzdem angreifen!" /></th></tr>';
	}
  ...
  $text.='<input type="submit" value="  Angriff !!  " /></th></tr>';
}
}
$text.='</table>';
  $a="pre_execute";
